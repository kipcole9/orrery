defmodule Orrery.CollectorFlagsTest do
  ...
  defp collect(repos) do
    report =
      Orrery.Collector.run(registry(repos), github?: false, cache_dir: System.tmp_dir!())
...
defmodule Orrery.CollectorForksTest do
  use ExUnit.Case, async: true
  ...
  test "a fork is left out of the report and listed as omitted" do
    ...
    report =
      Orrery.Collector.run(%{root: root, groups: [%{name: "T", dir: "."}], exclude: []},
        github?: false,
        hex?: false
      )
...
defmodule Orrery.CollectorBlockersTest do
  use ExUnit.Case, async: true
  ...
  test "a blocker satisfied by hex is reported as cleared" do
    repos = [
      ...
      %{name: "app", status: %{blockers: [Orrery.Status.blocker("dep ~> 1.3")]}, hex: nil}
    ]

    [_, app] = Orrery.Collector.resolve_blockers(repos, ~D[2026-09-21])
    assert [%{resolved: true, latest: "1.3.1", overdue: false}] = app.blocked.blockers
    refute app.blocked.active
  end
end

defmodule Orrery.CollectorOmissionPrecedenceTest do
  use ExUnit.Case, async: true

  alias Orrery.StubCollector

  defp with_github_fork(repo) do
    Map.put(repo, :github, %{fork: true, fork_of: "someone/thing", archived: false, ci: nil})
  end

  test "GitHub's fork flag omits a repository only when no STATUS.md speaks" do
    report = StubCollector.report()

    inferred = StubCollector.repo("stub/inferred") |> with_github_fork()

    explicit =
      StubCollector.repo("stub/explicit")
      |> with_github_fork()
      |> Map.merge(%{
        status: %{
          state: :active,
    ...
    result =
      report |> Orrery.Collector.replace(inferred) |> Orrery.Collector.replace(explicit)

    assert [%{name: "inferred", reason: "fork of someone/thing on GitHub"}] = result.omitted
    assert [%{repos: [%{name: "explicit"}]}] = result.groups
  end
end
