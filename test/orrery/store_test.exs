defmodule Orrery.StoreTest do
  ...
  alias Orrery.ScriptedCollector
  alias Orrery.Store
  alias Orrery.StubCollector
  ...
  test "collects on start, persists the report and broadcasts", %{data_dir: data_dir} do
    ...
    assert_receive {:orrery, :refreshed, status}
  ...
  test "a registry edited after the report is collected at start", %{data_dir: data_dir} do
    write_report(data_dir, DateTime.add(DateTime.utc_now(), -60, :second))
    registry = Path.join(data_dir, "projects.exs")
    File.write!(registry, ~s(%{root: "/tmp", groups: []}))

    start_store(data_dir, [{:ok, StubCollector.report()}],
      collector_options: [
        reply: fn -> {:ok, StubCollector.report()} end,
        test_pid: self(),
        projects_file: registry
      ]
    )

    assert_receive {:collecting, _}
    assert_receive {:orrery, :refreshed, _}
  end
  ...
  test "a persisted report older than the interval is replaced at start", %{data_dir: data_dir} do
    ...
    assert_receive {:collecting, _}
    assert_receive {:orrery, :refreshed, %{ready: true}}
  end
  ...
  test "refresh on demand, and a refresh already running is reused", %{data_dir: data_dir} do
    ...
    send(collector, :go)
    assert_receive {:orrery, :refreshed, %{refreshing: false}}
  ...
  test "a failed collection keeps the previous report and records the error", %{
    ...
    assert_receive {:orrery, :refresh_failed,
  ...
  test "a crashing collector is reported, not fatal", %{data_dir: data_dir} do
    ...
    assert_receive {:orrery, :refresh_failed, %{last_error: "collector crashed: " <> detail}}
    assert detail =~ "boom"
  ...
  test "collects again when the interval elapses", %{data_dir: data_dir} do
    ...
    assert_receive {:orrery, :refreshed, _}, 1_000
  end
end

defmodule Orrery.StoreScheduleTest do
  use ExUnit.Case, async: true

  doctest Orrery.Store

  test "the hourly window wraps to the next morning and never goes negative" do
    assert Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 20:30:00]) ==
             8 * 3_600_000 + 1_800_000

    assert Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 05:00:00]) == 3_600_000
    assert Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 04:59:59]) == 1_000
    assert Orrery.Store.next_delay({:hourly, 0..23}, ~N[2026-09-21 23:30:00]) == 1_800_000
  end
end

defmodule Orrery.StoreRepoRefreshTest do
  use ExUnit.Case, async: false
  ...
  setup do
    data_dir = Path.join(System.tmp_dir!(), "dashboard-one-#{System.unique_integer([:positive])}")
    File.mkdir_p!(data_dir)
    on_exit(fn -> File.rm_rf(data_dir) end)
    :ok = Store.subscribe()
    name = :"store_#{System.unique_integer([:positive])}"
    ...
    start_supervised!(
      Supervisor.child_spec(
        {Store,
         ...
         data_dir: data_dir,
         collector: ScriptedCollector,
         collector_options: [reply: fn -> {:ok, StubCollector.report()} end, test_pid: self()],
  ...
  test "one repository is collected and spliced in, and the schedule is untouched", %{
    store: store
  } do
    %{next_refresh_at: planned} = Store.status(store)

    assert Store.refresh_repo(store, "stub/one") == :started
    # The stub answers instantly, so the in-flight state is not observable here;
    # the broadcast carries the settled state.
    assert_receive {:collecting_one, "stub/one", _pid}
    assert_receive {:orrery, :refreshed, %{refreshing: false, refreshing_repo: nil}}

    assert {:ok,
            %{
              "groups" => [
                %{"name" => "Stub", "repos" => [%{"name" => "one", "path" => "stub/one"}]}
              ],
              "totals" => %{"repos" => 1}
            }} =
             Store.latest(store)

    assert %{next_refresh_at: ^planned} = Store.status(store)

    # Refreshing it again replaces rather than duplicates.
    assert Store.refresh_repo(store, "stub/one") == :started
    assert_receive {:orrery, :refreshed, _}
    assert {:ok, %{"totals" => %{"repos" => 1}}} = Store.latest(store)
  end

  test "a repository the registry does not know is reported, not fatal" do
    data_dir =
      Path.join(System.tmp_dir!(), "dashboard-unknown-#{System.unique_integer([:positive])}")

    File.mkdir_p!(data_dir)
    on_exit(fn -> File.rm_rf(data_dir) end)
    name = :"store_#{System.unique_integer([:positive])}"

    failing = fn -> {:ok, StubCollector.report()} end
    ...
    start_supervised!(
      Supervisor.child_spec(
        {Store,
         name: name,
         data_dir: data_dir,
         collector: Orrery.UnknownRepoCollector,
         collector_options: [reply: failing, test_pid: self()],
         refresh_interval: :timer.hours(24)},
        id: name
      )
    )
    ...
    assert_receive {:orrery, :refreshed, _}
    assert Store.refresh_repo(name, "nowhere/x") == :started

    assert_receive {:orrery, :refresh_failed,
                    %{last_error: "no repository at nowhere/x in the registry"}}

    assert {:ok, _} = Store.latest(name)
  end
end
