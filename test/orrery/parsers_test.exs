defmodule Orrery.ChangelogTest do
  use ExUnit.Case, async: true
  alias Orrery.Changelog
...
defmodule Orrery.GitVersionTest do
  use ExUnit.Case, async: true
  alias Orrery.Git
...
defmodule Orrery.IssueSummaryTest do
  use ExUnit.Case, async: true
  ...
  setup do
    ...
    github = %{
      issues: Enum.map(issues, &Orrery.GitHub.issue/1),
      prs: Enum.map(prs, &Orrery.GitHub.pull_request/1)
    }

    %{summary: Orrery.Collector.issue_summary(github, @now), github: github}
  end
...
defmodule Orrery.StandardTodoTest do
  use ExUnit.Case, async: true
  ...
  * [ ] **A mistake** — an unchecked box under Done is still open.
  ...
  test "state comes from the checkbox and the section" do
    {items, :checkbox} = Orrery.Plans.items(String.split(@todo, "\n"), task_file?: true)
  ...
  test "a plan with a Tasks section is tracked, and its prose is not" do
    ...
    * [ ] **Lazy set algebra** — union of generators.
    ...
    {items, :checkbox} = Orrery.Plans.items(String.split(plan, "\n"))
