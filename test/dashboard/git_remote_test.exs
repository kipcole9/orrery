defmodule Dashboard.GitRemoteTest do
...
defmodule Dashboard.GitTagForTest do
  use ExUnit.Case, async: true
  ...
  test "a tag in HEAD's history is reachable", %{dir: dir} do
    assert %{name: "v1.0.0", reachable: true, sha: sha, date: date} =
             Dashboard.Git.tag_for(dir, "1.0.0")
  ...
  test "a tag off the branch is found but not reachable, where describe misses it", %{dir: dir} do
    assert %{name: "v2.0.0", reachable: false} = Dashboard.Git.tag_for(dir, "2.0.0")
    assert Dashboard.Git.info(dir).last_tag == "v1.0.0"
  end

  test "no tag, or no version, is nil", %{dir: dir} do
    assert Dashboard.Git.tag_for(dir, "3.0.0") == nil
    assert Dashboard.Git.tag_for(dir, nil) == nil
  end
end
