defmodule OrreryWeb.OrreryController do
  @moduledoc """
  ...
  * `GET /data.json` returns the latest report.
  ...
  @doc """
  Starts a collection and answers `202 Accepted` with the current status.

  With a `repo` parameter (a path as the report shows it) only that
  repository is collected again. The store validates it against the
  registry, so the value is never used as a filesystem path here.
  """
  def refresh(conn, %{"repo" => path}) when is_binary(path) and byte_size(path) in 1..200 do
    result = Store.refresh_repo(path)
    send_json(conn, 202, %{result: result, repo: path, status: Store.status()})
  end
