defmodule Dashboard.Plans do
  ...
  defp block(block_lines) do
    ...
    if body != [] do
      candidate =
        header &&
          Enum.find_index(header, &Regex.match?(@status_headers, Dashboard.Text.plain(&1)))
      ...
      %{
        header: header,
        signature:
          header && Enum.map_join(header, "|", &String.downcase(Dashboard.Text.plain(&1))),
        body: body,
  ...
  defp table_item(cells, status_column) do
    ...
    title = rest |> Enum.map(&Dashboard.Text.plain/1) |> Enum.find(&(&1 != ""))
    ...
    if title do
      %{
        ref: ref,
        title: Dashboard.Text.truncate(title, 140),
        note: status_text |> Dashboard.Text.plain() |> Dashboard.Text.truncate(200),
  ...
  defp split_reference([first | rest] = cells) do
    plain = Dashboard.Text.plain(first)
  ...
  defp checkbox_item(line, section_state) do
    ...
    %{
      ref: nil,
      title: text |> Dashboard.Text.plain() |> Dashboard.Text.truncate(140),
      note: "",
      state: state
    }
  end
  ...
  defp classify_words(text) do
    plain = Dashboard.Text.plain(text)
  ...
  defp status_line(lines) do
    Enum.find_value(lines, fn line ->
      case Regex.run(~r/^\s*>?\s*\*\*Status:?\*\*:?\s*(\S.*)$/, line) do
        [_, t] -> t |> Dashboard.Text.plain() |> Dashboard.Text.truncate(260)
        _ -> nil
      end
    end)
  end
  ...
  defp headings(lines) do
    ...
    |> Enum.map(&(&1 |> String.replace(~r/^#+\s+/, "") |> Dashboard.Text.plain()))
    |> Enum.take(20)
  end
  ...
  defp modified(file, repo_path) do
    ...
    case Dashboard.Git.run(repo_path, ["log", "-1", "--format=%cI", "--", relative]) do
      nil -> nil
      "" -> nil
