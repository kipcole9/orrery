defmodule Dashboard.Projects do
  ...
  The file to read is `:projects_file` in the `:dashboard` application
  environment, or the `DASHBOARD_PROJECTS` environment variable at runtime.
  """
  ...
  * The configured `:projects_file`, or `priv/projects.exs` in the
  ...
  @spec file() :: Path.t()
  def file do
    Application.get_env(:dashboard, :projects_file) ||
      Path.join(:code.priv_dir(:dashboard), "projects.exs")
  end
  ...
  * `{:error, {:invalid, message}}` when the file does not evaluate to a
  ...
  ### Examples
      ...
      iex> {:error, {:missing_file, _}} = Dashboard.Projects.load("/nowhere/projects.exs")
  ...
  * `{:error, {:unknown_groups, names, known}}` when nothing matches.
  ...
  ### Examples
      ...
      iex> {:ok, %{groups: [%{name: "Tempo"}]}} = Dashboard.Projects.only(registry, ["tempo"])
      iex> Dashboard.Projects.only(registry, ["Nope"])
  ...
  * A string.
  ...
  ### Examples
      ...
      iex> Dashboard.Projects.describe({:missing_file, "/x/projects.exs"})
