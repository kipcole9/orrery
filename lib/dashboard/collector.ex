defmodule Dashboard.Collector do
  ...
  * `{:ok, report}` or `{:error, reason}` when the registry cannot be read;
    `Dashboard.Projects.describe/1` turns the reason into a message.

  """
  @spec collect(keyword()) :: {:ok, map()} | {:error, term()}
  def collect(options \\ []) do
    with {:ok, registry} <- Dashboard.Projects.load(Keyword.get(options, :projects_file)),
         {:ok, registry} <- Dashboard.Projects.only(registry, Keyword.get(options, :only, [])) do
  ...
  def run(registry, options \\ []) do
    ...
    data_dir = Dashboard.Store.default_data_dir()
    ...
    # still work, and Dashboard.HTTP.store/3 tolerates the failure.
    if github?, do: _ = Dashboard.GitHub.prepare(context.cache_dir)
    if context.hex?, do: _ = Dashboard.HTTP.prepare(context.hex_cache_dir)
    ...
    omitted = Enum.map(left_out, &%{path: &1.path, name: &1.name, reason: omit_reason(&1)})
  ...
  defp github_status(true) do
    budget = Dashboard.GitHub.Budget.state()

    %{
      enabled: true,
      authenticated: Dashboard.GitHub.token() != nil,
      token_var: Dashboard.GitHub.token_source(),
      calls: budget.calls,
  ...
  defp collect_repo(group, path, root, context) do
    git = Dashboard.Git.info(path)
    project = Dashboard.Git.project(path)
    changelog = Dashboard.Changelog.info(path)
    plans = Dashboard.Plans.scan(path)
    status = Dashboard.Status.info(path)
    status_state = Dashboard.Status.state(status)
    owner_repo = Dashboard.Git.owner_repo(git && git.remote)

    {github, github_error} =
      if context.github? and owner_repo do
        {owner, name} = owner_repo

        case Dashboard.GitHub.fetch(owner, name, context.cache_dir) do
          {:ok, data} -> {data, nil}
    ...
    remote = if context.remote? and git, do: Dashboard.Git.remote_state(path)
    version_tag = if git, do: Dashboard.Git.tag_for(path, project.version)

    {hex, hex_error} =
      if context.hex? and hex_package?(project, status_state) do
        case Dashboard.Hex.fetch(project.app, context.hex_cache_dir) do
          {:ok, data} -> {data, nil}
          {:error, reason} -> {nil, Dashboard.Hex.describe(reason)}
        end
      else
        {nil, nil}
      end
    ...
    state =
      git &&
        git
        |> Dashboard.Git.release_state(project.version)
  ...
  defp hex_release_state(:untagged, %{published: true, latest: latest}, version)
    ...
    case Dashboard.Git.compare(version, latest) do
      :gt -> :pending_release
      :lt -> :retagged
      :eq -> :released
      nil -> :untagged
  ...
  defp hex_release_state(:pending_release, %{published: true, latest: latest}, version)
    ...
    if Dashboard.Git.compare(version, latest) == :eq, do: :released, else: :pending_release
  end
  ...
  * The repositories with a `:blocked` map: `active` (any blocker still open)
  ...
  ### Examples
      ...
      ...> ]
      iex> [_, t] = Dashboard.Collector.resolve_blockers(repos, ~D[2026-09-21])
      iex> t.blocked
      ...
      iex> [_, t] = Dashboard.Collector.resolve_blockers(repos, ~D[2026-11-01])
  ...
  * Iodata.
  ...
  ### Examples
      ...
      iex> Dashboard.Collector.pretty_json(%{"a" => nil}) |> IO.iodata_to_binary()
      "{ \\"a\\": null }\\n"
  ...
  defp flags(repo) do
    ...
    [
      ...
      flag(
        activity? and (git && git.commits_90d == 0) and not archived?(repo),
        :info,
        "activity",
      ...
      flag(
        behind_hex?(repo) and not repo.pull.needed,
        :warn,
        "release",
      ...
      ci_flag(repo),
      flag(status not in [:active, :unknown], :info, "status", Dashboard.Status.label(status)),
      flag(
        status == :unknown,
        :warn,
        "status",
  ...
  defp behind_hex?(%{hex: %{published: true, latest: latest}, git: git} = repo)
    ...
    has_hex_tag? =
      match?(%{name: _}, repo.version_tag) and Dashboard.Git.compare(repo.version, latest) == :eq

    not has_hex_tag? and
      (git.last_tag_version == nil or Dashboard.Git.compare(git.last_tag_version, latest) == :lt)
  end
  ...
  defp published_untagged?(%{
       ...
       do: Dashboard.Git.compare(version, latest) == :eq
