defmodule Orrery.GitHub do
  ...
  and never written to the output. These names are tried in order:
      ...
      ORRERY_GITHUB_TOKEN    GITHUB_TOKEN    GH_TOKEN
  ...
  @token_vars ~w[ORRERY_GITHUB_TOKEN GITHUB_TOKEN GH_TOKEN]
  @per_page 100
  @max_pages 5
  ...
  * `:ok`, or `{:error, reason}` when the cache directory cannot be created.
  ...
  def prepare(cache_dir) do
    with :ok <- Orrery.HTTP.prepare(cache_dir),
         :ok <- ensure_budget() do
      Budget.reset()
    end
  end
  ...
  * A map with `:workflow`, `:status`, `:conclusion`, `:failed`, `:url`,
  ...
  ### Examples
      ...
      iex> Orrery.GitHub.workflow_run(%{"name" => "Elixir CI", "status" => "completed", "conclusion" => "failure", "html_url" => "https://github.com/o/r/actions/runs/1", "head_sha" => "abcdef0123456789", "head_branch" => "main", "display_title" => "Fix", "updated_at" => "2026-09-04T22:43:18Z"})
      ...
      iex> Orrery.GitHub.workflow_run(%{"status" => "in_progress", "conclusion" => nil}).failed
      false
  ...
  # ------------------------------------------------------------------
  # HTTP with ETag caching, through Orrery.HTTP
  # ------------------------------------------------------------------

  defp get_json(path, cache_dir) do
    url = @api <> path
    {cache_file, cached} = Orrery.HTTP.cached(cache_dir, url)
  ...
  defp request(url, cache_file, cached) do
    ...
    case Orrery.HTTP.get(url, headers, cached) do
      {:ok, 200, resp_headers, body} ->
        Budget.note(resp_headers)

        case Orrery.HTTP.decode_json(body) do
          {:ok, decoded} ->
            Orrery.HTTP.store(cache_file, resp_headers, decoded)
            {:ok, decoded}
