defmodule Orrery.Hex do
  ...
  `Orrery.HTTP`; hex allows 100 requests a minute unauthenticated, which
  ...
  * `{:error, reason}` when hex could not be asked and nothing is cached;
  ...
  def fetch(package, cache_dir) when is_binary(package) do
    if Regex.match?(@name, package) do
      ...
      case Orrery.HTTP.get(url, [], cached) do
        {:ok, 200, headers, body} ->
          case Orrery.HTTP.decode_json(body) do
            {:ok, decoded} ->
              Orrery.HTTP.store(cache_file, headers, decoded)
  ...
  * A string.
  ...
  ### Examples
      ...
      iex> Orrery.Hex.describe(:rate_limited)
