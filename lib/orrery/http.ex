defmodule Orrery.HTTP do
  @moduledoc """
  GET requests for JSON APIs over OTP's `:httpc`, with an ETag cache on disk.

  Shared by `Orrery.GitHub` and `Orrery.Hex`. Every successful response
  ...
  * `{:ok, term}` or `{:error, :invalid_json}`.
  ...
  ### Examples
      ...
      iex> Orrery.HTTP.decode_json(~s({"a": null}))
      {:ok, %{"a" => nil}}

      iex> Orrery.HTTP.decode_json("<html>")
      {:error, :invalid_json}
  ...
  * The value as a binary, or `nil`.
  ...
  ### Examples
      ...
      iex> Orrery.HTTP.find_header([{~c"ETag", ~c"abc"}, {~c"X-RateLimit-Remaining", ~c"59"}], "etag")
      "abc"

      iex> Orrery.HTTP.find_header([], "etag")
      nil
