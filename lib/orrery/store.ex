defmodule Orrery.Store do
  ...
  slot, or immediately when the registry file has changed since the report
  was made. A plain interval is available instead of the hourly window.
  ...
  Subscribers to `subscribe/0` receive `{:orrery, :refreshed, status}` after
  a successful collection and `{:orrery, :refresh_failed, status}` after a
  ...
  report; it defaults to `Orrery.Collector` and exists so tests can
  substitute one.
  """
  ...
  defstruct report: nil,
            ...
            data_dir: nil,
            collect_on_start: true,
            collector: Orrery.Collector,
            collector_options: []

  @type status :: %{
          ready: boolean(),
          refreshing: boolean(),
          refreshing_repo: String.t() | nil,
  ...
  * `:name` registers the process. Defaults to `Orrery.Store`.
  ...
  * `:started`, or `:already_running` when one is in flight; the running
  ...
  @doc """
  Collects one repository again and splices it into the current report,
  leaving the schedule alone.

  This is the cheap refresh: one repository's worth of git commands and
  GitHub and hex requests instead of the whole registry's. It falls back to a
  full collection when the store has only a report loaded from disk, since
  that form cannot be spliced.

  ### Arguments

  * `path` is the repository's path relative to the registry root, as shown
    in the report.

  ### Returns

  * `:started`, or `:already_running` when any collection is in flight.

  """
  @spec refresh_repo(GenServer.server(), String.t()) :: :started | :already_running
  def refresh_repo(server \\ __MODULE__, path) when is_binary(path) do
    GenServer.call(server, {:refresh_repo, path})
  end
  ...
  * An absolute path.
  ...
  def default_data_dir, do: Path.expand("~/.cache/orrery")
  ...
  @impl true
  def init(options) do
    options = Keyword.merge(Application.get_env(:orrery, __MODULE__, []), options)
    ...
    state = %__MODULE__{
      ...
      collector: Keyword.get(options, :collector, Orrery.Collector),
  ...
  # Either way, a registry edited after the report makes the report wrong.
  defp due?(%{generated_at: nil}), do: true

  defp due?(state) do
    registry_newer?(state) or slot_missed?(state)
  end

  defp registry_newer?(state) do
    file = Keyword.get(state.collector_options, :projects_file) || Orrery.Projects.file()

    case File.stat(file, time: :posix) do
      {:ok, %{mtime: mtime}} -> mtime > DateTime.to_unix(state.generated_at)
      _ -> false
    end
  end

  defp slot_missed?(%{schedule: {:every, ms}, generated_at: generated_at}) do
    DateTime.diff(DateTime.utc_now(), generated_at, :millisecond) >= ms
  end

  defp slot_missed?(%{schedule: {:hourly, hours}, generated_at: generated_at}) do
  ...
  * A non-negative integer.
  ...
  ### Examples
      ...
      iex> Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 09:15:00])
      2_700_000

      iex> Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 20:00:00])
      32_400_000

      iex> Orrery.Store.next_delay({:hourly, 5..20}, ~N[2026-09-21 03:59:59])
      3_601_000

      iex> Orrery.Store.next_delay({:every, 60_000}, ~N[2026-09-21 09:15:00])
      60_000
  ...
  def handle_call({:refresh_repo, _path}, _from, %{task: task} = state) when task != nil,
    do: {:reply, :already_running, state}

  # Without the collector's own form of the report there is nothing to splice
  # into, so the first refresh after a restart is a full one.
  def handle_call({:refresh_repo, _path}, _from, %{raw: nil} = state),
    do: {:reply, :started, start_collection(state)}

  def handle_call({:refresh_repo, path}, _from, state),
    do: {:reply, :started, start_repo_collection(state, path)}
  ...
  defp collector_options(state) do
    Keyword.merge(
      [
  ...
  defp start_repo_collection(state, path) do
    collector = state.collector
    options = collector_options(state)
    Logger.info("orrery: collecting #{path}")

    task =
      Task.Supervisor.async_nolink(Orrery.TaskSupervisor, fn ->
        collector.collect_one(path, options)
      end)

    %{
      state
      | task: task,
        refreshing_repo: path,
  ...
  defp start_collection(state) do
    ...
    collector = state.collector
    options = collector_options(state)
    Logger.info("orrery: collecting")

    task =
      Task.Supervisor.async_nolink(Orrery.TaskSupervisor, fn -> collector.collect(options) end)
  ...
  # A single repository comes back as a repo map and is spliced into the raw
  # report; the result is then handled exactly like a full report.
  defp finish({:ok, repo}, %{refreshing_repo: path} = state)
       when is_binary(path) and is_map(repo) do
    report = Orrery.Collector.replace(state.raw, repo)
    finish({:ok, report}, %{state | refreshing_repo: nil})
  rescue
    exception ->
      finish({:error, {:replace, Exception.message(exception)}}, %{state | refreshing_repo: nil})
  end
  ...
  defp finish({:ok, report}, state) when is_map(report) do
    ...
    Logger.info(
      "orrery: collected #{normalised["totals"]["repos"]} repositories in #{div(duration, 1000)}s"
    )

    state = keep_or_schedule(state)
  ...
  defp finish({:error, reason}, state) do
    ...
    Logger.error("orrery: collection failed: #{message}")
  ...
  # A full collection resets the schedule from now (start_collection cancelled
  # the timer); a single-repository one leaves the pending slot alone, unless
  # the slot fired meanwhile and was swallowed, in which case the next one is
  # set up.
  defp keep_or_schedule(%{timer: nil} = state), do: schedule(state, next_delay(state.schedule))
  defp keep_or_schedule(state), do: state
  ...
  defp describe({:replace, message}), do: "report could not be updated: " <> message
  defp describe({:unknown_repo, path}), do: "no repository at #{path} in the registry"
  defp describe({:collect, message}), do: "collecting the repository failed: " <> message
  ...
  defp describe(reason), do: Orrery.Projects.describe(reason)
  ...
  defp persist(state) do
    ...
    with :ok <- File.mkdir_p(state.data_dir),
         :ok <- File.write(file, Orrery.Collector.pretty_json(state.report)) do
      :ok
    else
      {:error, reason} ->
        Logger.warning("orrery: could not write #{file}: #{:file.format_error(reason)}")
    end
  end
  ...
  defp load_persisted(state) do
    ...
    with {:ok, raw} <- File.read(file),
      ...
      Logger.info("orrery: loaded report from #{file}, generated #{generated}")
      ...
      other ->
        Logger.warning("orrery: ignoring #{file}: #{inspect(other, limit: 3)}")
        state
    end
  end
  ...
  defp status_of(state) do
    %{
      ...
      refreshing_repo: state.refreshing_repo,
  ...
  defp broadcast(event, state) do
    Phoenix.PubSub.broadcast(@pubsub, @topic, {:orrery, event, status_of(state)})
  end
end
